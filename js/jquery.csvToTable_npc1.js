	...
	$.fn.CSVToTable = function(csvFile, options) {
		...
		return this.each(function() {
			...
			$.get(csvFile, function(data) {
				var tableHTML = '<table class="' + options.tableClass + ' box">';
				...
				$.each(lines, function(lineCount, line) {
					if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
						...
						tableHTML += '<thead class="' + options.theadClass + ' heads"><tr class="' + options.trClass + ' heads">';
					...
					if ((lineCount == options.startLine) && (typeof(options.headers) == 'undefined')) {
						...
						headerCount = headers.length;
						tableHTML += '<thead id="head" class="' + options.theadClass + '"><tr class="' + options.trClass + ' heads">';
						$.each(headers, function(headerCount, header) {
							tableHTML += '<th class="' + options.thClass + '">' + header + '</th>';
						});
						...
						if (items.length > 1) {
							...
							//tableHTML += '<tr style="background:' + theColor + ';" class= "' + options.trClass + ' ' + oddOrEven + ' rows" id= "' + items[0] + '">';
							tableHTML += '<tr class= "' + options.trClass + ' ' + oddOrEven + ' rows" id= "' + items[0] + '">';
							$.each(items, function(itemCount, item) {
								values.push(item);
								tableHTML += '<td style="background:' + colors[item.trim()] + '" class="' + options.tdClass + '">' + item + '</td>';
							});
				...
				tableHTML += '<tr class= "empty rows ' +  options.trClass + '" id ="empty">'
				tableHTML += '<td> --- </td>'
				tableHTML += '<td> --- </td>'
				tableHTML += '<td> --- </td>'
				tableHTML += '<td> --- </td>'
